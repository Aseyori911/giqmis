      ...
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-3xl mx-auto">
          {faqs.map(({ id, question, answer }) => (
            <div key={id} className="mb-4 rounded-lg overflow-hidden border border-gray-200">
              <input type="checkbox" id={id} className="absolute opacity-0 -z-10 peer" />
              <label htmlFor={id} className="flex justify-between p-5 bg-gray-50 cursor-pointer items-center peer-checked:[&>.accordion-icon]:rotate-45">
                <h4 className="text-slate-800 text-lg m-0">{question}</h4>
              ...
              <div className="max-h-0 overflow-hidden transition-all duration-300 ease-in-out bg-white peer-checked:max-h-48">
                <p className="p-5 leading-relaxed text-slate-500">{answer}</p>
              </div>
            </div>
          ))}
        </div>
