          ...
          <div className="relative max-w-3xl mx-auto">
            ...
            {journeyMilestones.map(({ year, title, description }, i) => {
              ...
              return (
                <div
                  key={i}
                  className={`relative w-1/2 mb-10 ${isLeft ? "pr-10" : "pl-10 ml-auto"}`}
                >
                  <div
                    className={`absolute w-5 h-5 bg-white dark:bg-stone-900 border-4 border-orange-500 rounded-full top-5 z-10 ${isLeft ? "-right-2.5" : "-left-2.5"}`}
                  ></div>
                  <div className="p-5 bg-gray-50 dark:bg-stone-800 rounded-lg shadow-sm">
                    <h4 className="text-lg text-orange-500 mb-2">{year}</h4>
                    <h3 className="text-xl text-slate-800 dark:text-stone-100 mb-2">
                      {title}
                    </h3>
